...
export class NuxtUtil {
  ...
  /**
   * Generate module stores from folder & config file
   */
  registerStores() {
    this.registerStore('', '');

    if (this.config.stores && this.config.stores.length > 0) {
      this.config.stores.forEach((element: any) => {
        if (Object.keys(element).length !== 0) {
          let o = {
            src: path.resolve(this.dirname, 'store/' + element.src),
        ...
        files.forEach((file: any) => {
          if (!!path.extname(file)) {
            ...
            if (this.i18n && this.i18n.langs) {
              this.i18n.langs.forEach((lang: any) => {
                ...
                if (this.i18n.except.indexOf(lang)==-1) {
                  let o = {
                    name: (p + (file != 'index.vue' ? '/' + path.parse(file).name : '')).replace('/', '').replace(/\//ig, '-').replace(/_/g, '-').replace(/\[/ig, '').replace(/\]/ig, '') +`___${lang}`,
            ...
            // if no i18n module
              ...
              let o = {
                name: (p + (file != 'index.vue' ? '/' + path.parse(file).name : '')).replace('/', '').replace(/\//ig, '-').replace(/_/g, '-').replace(/\[/ig, ':').replace(/\]/ig, ''),
