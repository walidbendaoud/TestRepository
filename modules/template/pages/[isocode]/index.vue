  ...
  <div id="components-button-demo-button-group">
    ...
    >
      <img
        ...
        :src="instance.proxy.$config.MEDIA + preview.value"
      >
      ...
      <template slot="aggregators" slot-scope="aggregators">
        <ul v-if="aggregators !== null && aggregators.length && typeof aggregators != 'string'" class="aggregators">
          <li v-for="item in aggregators" :key="item.id">
  ...
  setup () {
    ...
    const useRemoveItem = (record: any) => {
      store.useTemplateDelete({
        $id: record.id,
        onError: () => {
          useOpenNotification(record.type, 'alert')
        },
        onSuccess: () => {
...
@import '@TEMPLATE/assets/scss/templateList.scss';
</style>
