    ...
    <a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }" @submit.prevent="useHandleSubmit">
      ...
      <a-form-item :label="'*' + tw.t('preview')">
        <div class="d-flex">
          <div v-if="previewUrl !== ''" class="previewBlock">
            <img :src="imgUrlLink + previewUrl" alt="">
            <a-icon type="close" @click="useDeletePreview" />
          </div>
          <template-Media v-else :is-media="false" :is-visible="false" @change-preview="useChangePreview" />
        </div>
      </a-form-item>
      ...
      <a-form-item label="Type">
        ...
        >
          ...
          <a-select-option :value="tw.t('MULTISERVICE')">
            {{ tw.t('MULTISERVICEM') }}
          </a-select-option>
        </a-select>
      </a-form-item>
      ...
      <template-Media
        ...
        :image-url-array-src="imageUrlArray"
        :is-visible="visible"
        :is-media="true"
        @change-body="usechangeBody"
        @change-visible="useChangeVisible"
        @change-imageArray="useChangeImageArray"
      />
        ...
        <div class="bodyContent">
          ...
          <div class="bodyRow">
            <v-md-editor
              ...
              height="400px"
            />
            <template-params class="tempParams" @change-titleParam="useTitleParam" />
          </div>
        </div>
      </div>
      ...
      <a-form-item :label="tw.t('aggrigatorFormLabel')">
        <a-checkbox :checked="aggreCheck && selectedRowKeysAggrList.length === 0" @change="useOnChangeAggregator" />
      </a-form-item>
      ...
      <a-form-item :label="tw.t('backgroundRecommandFormLabel')">
        <a-input v-decorator="['bgRecommandationColor']" :disabled="id !== undefined" @input="useHandleChange" />
      </a-form-item>
...
import templateParams from './params.vue'
...
export default defineComponent({
  ...
  setup () {
    ...
    const imgUrlLink = instance.proxy.$config.MEDIA
      ...
      if (id.value) {
        store.useTemplateItem({
          ...
          onSuccess: (e: any) => {
            ...
            previewUrl.value = e.preview.value
            selectedRowKeysAggrSearch.value = e.aggregators
            selectedRowKeysAggrList.value = e.aggregators
            if (selectedRowKeysAggrSearch.value.length > 0) {
    ...
    const useTitleParam = (val: any) => {
      usechangeBody(val)
    }

    const useDeletePreview = () => {
      preview.value = ''
      previewUrl.value = ''
    }
    ...
    const useChangeImageArray = (val: any, val2: any) => {
      imageNameArray = val
      imageUrlArray = val2
    }
